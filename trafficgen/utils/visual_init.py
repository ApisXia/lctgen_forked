...
def get_heatmap(x, y, prob, s, bins=1000):
    heatmap, xedges, yedges = np.histogram2d(
        x, y, bins=bins, weights=prob, density=True
    )
...
def draw_seq_map(
    ...
    if save:
        ...
        fig.tight_layout()
        fig.canvas.draw()
        return np.fromstring(fig.canvas.tostring_rgb(), dtype=np.uint8, sep="").reshape(
...
def draw(
    ...
    for i in range(len(agents)):
        ...
        rect = agent.get_rect()[0]
        rect = plt.Polygon(
            rect, edgecolor=lane_color, facecolor=col, linewidth=0.5, zorder=10000
        )
        if abs(vel[0] + center[0]) < (vis_range - 2) and abs(vel[1] + center[1]) < (
            vis_range - 2
        ):
            ax.plot(
                [center[0], vel[0] + center[0]],
                [center[1], vel[1] + center[1]],
                ".-",
                color="lime",
                linewidth=1,
                markersize=2,
