...
def visualize_decoder(data, decode_probs):
    ...
    heatmap = get_heatmap(
        coords[:, 0][center_mask], coords[:, 1][center_mask], prob[center_mask], 20
    )

    return draw(
        center, [], other=rest, edge=bound, save_np=True, showup=False, heat_map=heatmap
    )
...
def visualize_input(data, agents=None):
    ...
    if agents is None:
        ...
        agents = [
            WaymoAgent(agent[i : i + 1]) for i in range(agent.shape[0]) if agent_mask[i]
        ]
...
def visualize_query_heatmaps(data, output):
    ...
    for i in range(max_num):
        heatmap = get_heatmap(
            center[:, 0][center_mask], center[:, 1][center_mask], probs[i], smooth, size
        )
        figures.append(
            draw(
                center,
    ...
    for row in range(ROWS):
        figure_rows.append(
            np.concatenate(figures[row * COLS : (row + 1) * COLS], axis=1)
        )
...
def transform_traj_output_to_waymo_agent(output, fps=10):
    ...
    if not pred_future_heading:
        ...
        for i in range(len(traj) - 1):
            ...
            traj_headings.append(
                np.arctan2(end[:, 1] - start[:, 1], end[:, 0] - start[:, 0])
            )
            traj_speeds.append(np.linalg.norm(end - start, axis=1) * fps)

        init_speeds = np.array(
            [np.linalg.norm(np.linalg.norm(agent.velocity[0])) for agent in init_agent]
        )
...
def visualize_output_seq(data, output, fps=10, pool_num=16):
    ...
    with multiprocessing.Pool(pool_num) as pool:
        draw_frame_partial = partial(
            draw_frame, output_scene=output, pred_agents=pred_agents, data=data
        )
